...
std::tuple<int, int, std::vector<Color>> read_image(const std::string& file_name) {
    ...
    if (!file.is_open()) {
        std::cerr << "Nie można otworzyć pliku " << file_name << std::endl;
        return {0, 0, {}};
    }
...
int main() {
    ...
    do {
        ...
        std::cout << "Najczęściej występujący kolor to " << most_common_color->first.r << "-"
                  << most_common_color->first.g << "-" << most_common_color->first.b
                  << " i wystąpił " << most_common_color->second << " razy" << std::endl;

        std::cout << "Liczba unikalnych kolorów: " << color_counts.size() << std::endl;

        std::cout << "\nCzy chcesz wczytać kolejny plik (tak/nie): ";
        std::cin >> choice;
